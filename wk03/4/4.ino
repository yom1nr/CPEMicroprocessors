...
void loop() {
  ...
  if (client) {
    ...
    // 1. ส่วนส่งข้อมูล JSON (Telemetric Data) สำหรับอัปเดตหน้าเว็บอัตโนมัติ
    if (request.indexOf("GET /data") != -1) {
      float t = dht.getTemperature();
      float f = dht.toFahrenheit(t); // สำหรับรหัสคี่
    ...
    // 3. ส่วนส่งหน้าเว็บหลักสไตล์ SpaceX
    else {
      client.println("HTTP/1.1 200 OK");
