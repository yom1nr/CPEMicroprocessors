...
void handleLED() {
  ...
  if (t_state == "81") {
    digitalWrite(testLED8, LOW);  //Feedback parameter
    ledState8 = "ON";
  }
  if (t_state == "80") {
    digitalWrite(testLED8, HIGH);  //Feedback parameter
    ledState8 = "OFF";
  }
  server.send(200, "text/plane", ledState1 + ", " + ledState2 + ", " + ledState3 + ", " + ledState4  + ", " + ledState5 + ", " + ledState6 + ", " + ledState7 + ", " + ledState8); //Send web page
}
void setup(void) {
  Serial.begin(115200);
  ...
  pinMode(testLED5, OUTPUT);
  pinMode(testLED6, OUTPUT);
  pinMode(testLED7, OUTPUT);
  pinMode(testLED8, OUTPUT);
  Serial.print("\n\nConnect to ");
  Serial.println(ssid);
